...
COMMON_CFLAGS   = -Wall -Wextra -O3 -g
...
DEFINES         :=
INCLUDES        := -Isource/include $(shell pkg-config --cflags libevdev x11)

LIBS            := $(shell pkg-config --libs libevdev x11)

OUTPUT_BIN      := build/xkeyslug
...
$(OUTPUT_BIN): $(CXXOBJ)
	@echo "  $(notdir $@)"
	@mkdir -p build
	@$(CXX) $(CXXFLAGS) $(WARNINGS) $(DEFINES) -o $@ $^ $(LIBS)
